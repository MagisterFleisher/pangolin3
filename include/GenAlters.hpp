...
Altermap GenAlters(const Graph& g, const Node& node) {
    ...
    Altermap altermap;
    std::for_each(std::execution::unseq, cRANGE(g.edges), [&node, &g, &altermap](const auto& edge) {
        if(node == edge.from)   {   altermap.emplace(std::pair(edge.from, edge.to  )); }
        if(node == edge.to)     {   altermap.emplace(std::pair(edge.to  , edge.from)); } });
    std::cout << "\tFinished Altermap\n"; 
    return altermap; }
